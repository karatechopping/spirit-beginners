...
if (!file_exists('../config/mail-config-kelburn.php')) {
    http_response_code(500);
    echo "Mail configuration not found. Please create mail-config-kelburn.php in config/ folder";
    exit;
}
require_once('../config/mail-config-kelburn.php');
...
try {
    ...
    if ($adminEmailSent && $confirmEmailSent) {
        ...
        // Redirect back to the page they came from, or fallback to index-kelburn.html
        $referer = $_SERVER['HTTP_REFERER'] ?? 'index-kelburn.html';
        $separator = (strpos($referer, '?') !== false) ? '&' : '?';
        header("Location: {$referer}{$separator}success=1#contact");
        exit;
    }
